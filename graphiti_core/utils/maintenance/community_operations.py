...
async def generate_summary_description(llm_client: LLMClient, summary: str, ensure_ascii: bool = True) -> str:
    context = {'summary': summary, 'ensure_ascii': ensure_ascii}
...
async def build_community(
    ...
    while length > 1:
        ...
        new_summaries: list[str] = list(
            await semaphore_gather(
                *[
                    summarize_pair(llm_client, (str(left_summary), str(right_summary)), ensure_ascii)
...
async def update_community(
    driver: GraphDriver, llm_client: LLMClient, embedder: EmbedderClient, entity: EntityNode, ensure_ascii: bool = True,
) -> tuple[list[CommunityNode], list[CommunityEdge]]:
    ...
    new_summary = await summarize_pair(llm_client, (entity.summary, community.summary), ensure_ascii)
